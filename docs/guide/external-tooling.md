...
| **Gives the canvas the viewport** | Every shell region that is not the canvas — header, sidebar, view tabs, component head, variant ribbon, the addon panel and its resizer, the canvas toolbar — is removed from layout, and the stage loses its padding, so the canvas is exactly the window. This is what makes the declared background reach the _whole_ of `.demo-wrap`'s box; see [the guarantee](#the-guarantee-and-its-one-limit).                                                 |
