...
| **Gives the canvas the viewport** | Every shell region that is not the canvas — header, sidebar, view tabs, component head, variant ribbon, the addon panel and its resizer, the floating canvas rail — is removed from layout, and the stage loses its padding, so the canvas is exactly the window. This is what makes the declared background reach the _whole_ of `.demo-wrap`'s box; see [the guarantee](#the-guarantee-and-its-one-limit).                                           |
...
Capture mode's promise is that **`.demo-wrap` is composited against its declared background over the whole of its box** — including the part that would scroll out of the canvas while browsing. That guarantee is the reason a runner may screenshot `.demo-wrap` directly instead of reconstructing the component's bounds itself.

Until 22.2.0-beta.4 the shell was left in place, on the reasoning that it "sits outside `.demo-wrap` and never appears in an element-scoped screenshot". That reasoning is wrong, and it is worth knowing why, because the same mistake is easy to repeat in a consumer's own CSS. The component is centred in a canvas that _scrolls_. A screenshot tool captures the **screen coordinates** of the target's box, not the part of it the canvas happens to show — so wherever the box extends past the canvas, the image picks up whichever region paints at those coordinates. Measured in Chromium at a 1280x720 viewport, a 200x400 component leaked 57px upwards into the variant ribbon and 121px downwards into the addon panel. It is neither a z-order nor a clipping problem: the capture target legitimately extends beyond its scroll container, and the page paints something there.

The limit is what no stylesheet can fix: capture mode cannot make room the window does not have. The canvas _is_ the viewport, so a component larger than the viewport still overflows it, and the overflowing part is composited against whatever lies outside the canvas. **Size the runner's viewport to the largest variant you baseline.**

### What it does not change

Your component is instantiated with the same inputs, in the same injector, at the same zoom, in the same document. The shell regions capture mode drops are removed from _layout_, not unmounted — nothing about the component's own box depends on them. A view with no canvas at all (a [component page](guide/component-pages.md), a plugin view panel) is left exactly as it renders normally; capture mode's guarantee is about the canvas, and blanking the shell around a page would trade a contaminated screenshot for an empty one.

If you want to strip the shell for an embedding scenario rather than for a screenshot, that is a different concern; see [Custom UI Sections](guide/custom-ui.md).
