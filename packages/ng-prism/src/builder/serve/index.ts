...
async function createServeBuilder(
  ...
  const configFilePath = join(
    ...
    pipelineOptions.configFile
  );
  const absolutePrismProject = join(
    context.workspaceRoot,
    options.prismProject
  );
...
const builder: Builder<ServeBuilderSchema & json.JsonObject> =
  createBuilder<ServeBuilderSchema>(createServeBuilder);
